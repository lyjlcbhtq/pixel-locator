...
**Three ways to get the code** (the ZIP is generated by GitHub automatically — the author
does not upload any archive):

| Way | Action / URL | Best for |
|---|---|---|
| **Download ZIP** | green **`Code`** button on the repo page → **Download ZIP** | no git needed, simplest |
| **git clone** | `git clone https://github.com/lyjlcbhtq/pixel-locator.git` | following updates |
| **Pinned version** | `https://github.com/lyjlcbhtq/pixel-locator/archive/refs/tags/v1.0.0.zip` | reproducible builds |

**Then let the one-click setup script do the rest**:

```bat
install.bat          :: Windows — just double-click it
```
...
The script does four things: check Python → install dependencies → environment check →
smoke test. (Dependencies include the OCR model, so the first install needs access to
pypi.org.)

**Requirement**: Python **3.10 or newer**. On Windows, tick **"Add Python to PATH"** during
installation.
...
├── install.bat / install.sh  One-click setup (Python check → deps → checks → smoke)
