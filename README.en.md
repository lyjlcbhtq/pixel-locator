...
> **Pixel-precise screen coordinates for AI agents.**
> Let the vision model understand the interface; let OCR and template matching deliver the exact click point.
> Pure CPU · Zero GPU · Zero API tokens on the main path · Fully usable **without any vision model**.
...
├── smoke.py                Smoke test (bundled fixtures, runs right after clone)
